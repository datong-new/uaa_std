...
parser.add_argument('--tune_from', default='/data/shudeng/text_attack/attacks/Text_Detector/ICDAR2015_TextBoxes.pth', type=str, help='pre-trained weight')
parser.add_argument('--output_zip', default='IC15_result_1', type=str, help='evaluation zip output')
parser.add_argument('--save_img', default=True, type=str2bool, help='save output image')
...
img_dir = "/data/shudeng/IC15/ch4_test_images/" if args.dataset in ["ICDAR2015"] else "/root/DB/ICDAR2013_FOCUSED/test/"
...
#    print("infer : %d / %d" % (n, len(val_list)), end='\r')
    ...
    print("scale", input_scale)
    print("loc shape", loc_preds.shape)
    ...
    if quad_boxes.shape[0] is 0:
        print("none")
        print("quad_boxes shape", quad_boxes.shape)
        continue
    ...
    if args.save_img:
        ...
        save_img_dir = "/data/shudeng/text_attack/attacks/res_textbox++/%s_%s"
        print(save_img_dir % ("img", _img))
